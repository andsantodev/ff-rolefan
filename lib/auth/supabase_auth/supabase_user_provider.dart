...
class RoleFanSupabaseUser extends BaseAuthUser {
  RoleFanSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> roleFanSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = RoleFanSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
